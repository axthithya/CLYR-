...
| Trusted tool command / 7 | Per supported vendor/version | Compiled first-party adapter | Per exact operation | Verified executable, typed args, timeout/output limits | Core offline; tool network effect disclosed | Missing/unknown/parse ambiguity becomes report-only | Fake runner plus disposable vendor matrix | Planned later |
| Developer storage report / 7 | Per tool/version | App identity | No | Official discovery/version interfaces | Full for detection | Report-only/Unknown; never delete volumes/disks/dependencies | Positive/negative/version/locale fixtures | Planned later |
