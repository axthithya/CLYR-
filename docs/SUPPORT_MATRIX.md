...
| Cleanup plan/dry run | Current validated Windows lane | Built-in report-only eligible metadata; fixed NTFS source | Current unpackaged developer identity | No | Typed contracts/protected policy | Full | Reject stale, duplicate, ambiguous, protected targets; execution unavailable | Phase 5 malicious-plan/schema/CLI/UI fixtures | Implemented preview; not cleanup execution |
