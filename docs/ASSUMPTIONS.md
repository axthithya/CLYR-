...
| A-004 | The latest patched .NET 10 LTS and a stable Windows App SDK will be selected and pinned in Phase 1. | Phase 0 has no SDK project, and the local host has no .NET SDK. Exact package versions must be verified at scaffolding time. | Start of Phase 1 |
...
| 3 | Which WinUI 3/Windows App SDK stable version and packaging mode pass build, launch, signing, and upgrade tests? | High: architecture and delivery. | No support claim; Platform owner. | Phase 1 |
...
| 7 | Is reliable WinUI automation available for the chosen package topology? | Medium: release confidence. | Documented manual smoke test; Test owner. | Phase 1 |
