...
public sealed class ElevatedMetadataRetryEngine(IFileSystemEnumerator fileSystem, IClock clock)
{
    ...
    private ElevatedScanRetryResponse Retry(ElevatedScanRetryRequest request, DateTimeOffset startedAtUtc, CancellationToken cancellationToken)
    {
        ...
        try
        {
            foreach (var root in request.PermissionLimitedRoots)
            {
                ...
                var completed = EnumerateRoot(root, accumulator, cancellationToken, out var cancelledDuringRoot);
                if (cancelledDuringRoot) { cancelled = true; break; }
        ...
        catch (Exception exception) when (!IsExpected(exception))
        {
            ...
            return Terminal(request, ElevatedScanRetryOutcome.Failed, startedAtUtc, clock.UtcNow, accumulator, rootsCompleted, rootsStillInaccessible);
        }
        ...
        return Terminal(request, outcome, startedAtUtc, clock.UtcNow, accumulator, rootsCompleted, rootsStillInaccessible);
    }
    ...
    /// inventory (only <see cref="Accumulator"/>'s running totals grow). Returns <see langword="true"/> only if
    /// the root's own subtree was walked to exhaustion without cancellation; a root that could not even be
    /// opened, or whose enumeration hit an ordinary access/IO condition partway through, still returns
    /// <see langword="false"/> (recorded as "still inaccessible") rather than throwing — this is what lets the
    /// caller continue on to the next root.</summary>
    private bool EnumerateRoot(PermissionLimitedRoot root, Accumulator accumulator, CancellationToken cancellationToken, out bool cancelledDuringRoot)
    {
        ...
        {
            accumulator.RecordDiagnostic(ReasonCodeFor(exception), SeverityFor(exception), label);
            return false;
        }
        ...
        try
        {
            while (stack.Count > 0)
            {
                if (cancellationToken.IsCancellationRequested) { cancelledDuringRoot = true; return false; }
                ...
                {
                    accumulator.RecordDiagnostic("root.reparse-point-skipped", ScanIssueSeverity.Information, label);
                    continue;
                }
                if ((entry.Traits & EntryTraits.Directory) != 0)
                {
                    accumulator.DirectoriesExamined++;
                ...
                accumulator.RecordFile(entry);
            }
        }
        finally { while (stack.Count > 0) stack.Pop().Dispose(); }
        return rootFullyOpened;
    }
    ...
    private static ElevatedScanRetryResponse Terminal(ElevatedScanRetryRequest request, ElevatedScanRetryOutcome outcome,
        ...
        new(request.ProtocolVersion, request.Nonce, outcome, startedAtUtc, completedAtUtc,
            ...
            accumulator.SparseFileCount, accumulator.CompressedFileCount, accumulator.FormatDiagnostics());
    ...
    private sealed class Accumulator(int maximumDiagnosticCount)
    {
        ...
        public void RecordFile(FileSystemEntry entry)
        {
            ...
            if (allocated is null) RecordDiagnostic("allocation.unavailable", ScanIssueSeverity.Information, "multiple");
        }
